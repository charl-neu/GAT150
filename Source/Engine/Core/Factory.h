...
namespace viper {
	...
	inline void Factory::Register(const std::string& name) {
		...
		//log registration
		Logger::Info("Factory: Registered creator for type: {}", key);
	}
	...
	inline std::unique_ptr<T> Factory::Create(const std::string& name) {
		...
		if (it != m_registry.end()) {
			//found creator, call create method
			...
			Logger::Error("Factory: Type mismatch of factory object: {}", key);
		}
		else {
			//if not found, log error
			Logger::Error("Factory: No creator registered for type: {}", key);
		}
		return nullptr;
	}
}